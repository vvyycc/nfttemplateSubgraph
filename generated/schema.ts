...
export class CreatedNFT extends Entity {
  ...
  save(): void {
    ...
    if (id) {
      assert(
        id.kind == ValueKind.BYTES,
        `Entities of type CreatedNFT must have an ID of type Bytes but the id '${id.displayData()}' is of type ${id.displayKind()}`
      );
      store.set("CreatedNFT", id.toBytes().toHexString(), this);
    }
  }

  static loadInBlock(id: Bytes): CreatedNFT | null {
    return changetype<CreatedNFT | null>(
      store.get_in_block("CreatedNFT", id.toHexString())
    );
  }

  static load(id: Bytes): CreatedNFT | null {
    return changetype<CreatedNFT | null>(
      store.get("CreatedNFT", id.toHexString())
    );
  }
